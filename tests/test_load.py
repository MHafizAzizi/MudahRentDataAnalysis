...
class TestUpsertDataframe:
    def test_inserts_rows(self, in_memory_conn, sample_df):
        count = load_to_db.upsert_dataframe(in_memory_conn, sample_df)
        assert count == 3

    def test_rows_in_db(self, in_memory_conn, sample_df):
        load_to_db.upsert_dataframe(in_memory_conn, sample_df)
        result = pd.read_sql("SELECT * FROM properties", in_memory_conn)
        assert len(result) == 3

    def test_upsert_replaces_existing(self, in_memory_conn, sample_df):
        load_to_db.upsert_dataframe(in_memory_conn, sample_df)
    ...
    def test_upsert_preserves_recheck_state(self, in_memory_conn):
        df = pd.DataFrame({'ads_id': ['020'], 'scrape_date': ['2026-05-01'], 'monthly_rent': [1000.0]})
        load_to_db.upsert_dataframe(in_memory_conn, df)
        in_memory_conn.commit()
        # Simulate recheck having marked it gone.
        in_memory_conn.execute(
            "UPDATE properties SET availability_status='rented', gone_at='2026-05-10', "
            "last_checked_at='2026-05-10' WHERE ads_id='020'"
        )
        in_memory_conn.commit()
        # Re-scrape later: listing reappears -> re-affirmed active, first_seen kept.
        df2 = pd.DataFrame({'ads_id': ['020'], 'scrape_date': ['2026-05-20'], 'monthly_rent': [1100.0]})
        load_to_db.upsert_dataframe(in_memory_conn, df2)
        in_memory_conn.commit()
    ...
    def test_missing_ads_id_raises(self, in_memory_conn):
        bad_df = pd.DataFrame({'state': ['Selangor'], 'monthly_rent': [1000.0]})
        with pytest.raises(ValueError, match="ads_id"):
            load_to_db.upsert_dataframe(in_memory_conn, bad_df)

    def test_missing_columns_filled_with_none(self, in_memory_conn):
        minimal_df = pd.DataFrame({'ads_id': ['999'], 'monthly_rent': [1000.0]})
        count = load_to_db.upsert_dataframe(in_memory_conn, minimal_df)
        assert count == 1
